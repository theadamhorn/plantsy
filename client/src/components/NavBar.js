...
export default function NavBar() {
    ...
    if (authData.isAuthenticated) {
        ...
        <Link to="/users" className="nav-link col">My Plants</Link>
        <Link to="/trellis" className="nav-link col">Trellis</Link>
    </div>
        log_in_out_Link= <div><a class="nav-link active" aria-current="page" data-bs-toggle="modal" href="#modal" role="button" onClick={logout}>Logout <FontAwesomeIcon icon={faSignOutAlt} className="fa-lg" /></a></div>
    } else {
        links =''
        
        log_in_out_Link =<div>
        <a className="nav-link active" aria-current="page" data-bs-toggle="modal" href="#modal" role="button">Login <FontAwesomeIcon icon={faSignInAlt} className="fa-lg" /></a>
        <LoginModal/>
        </div>
    }
